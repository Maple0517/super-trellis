...
8. ** impact**（`08-`） — index stale，未实跑 `impact()`，但 grep 等价分析显示 risk MEDIUM（条件只收紧不放宽），byte-identity 保护已足够；建议 implement agent 跑一次全仓 grep `resolveTrellisStartSkill|resolveCodexTrellisStartSkill` 兜底。
...
- 没动 `.trellis/spec/`
- 没跑  `impact()` MCP 工具（index stale + grep 已足够）
