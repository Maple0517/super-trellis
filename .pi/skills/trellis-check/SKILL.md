...
For symbol-level claims, cross-layer claims, caller/callee relationships, flow tracing, or impact-radius review, run tool preflight first. In Codex, inspect nested/deferred tools from inside `exec` by checking `ALL_TOOLS` for CodeGraph and LeanCTX MCP tools. Verify structural claims with CodeGraph MCP when exposed. Use LeanCTX MCP or `/Users/maple/.local/bin/lean-ctx -c "<cmd>"` for fresh diff/status/test output, compressed logs, file reads, repo overview, routes, semantic search, and session memory. If CodeGraph MCP is not exposed, say `CodeGraph tools are not exposed this turn` before falling back to LeanCTX or native text search; if CodeGraph is exposed but stale or degraded, say so and use LeanCTX fresh reads or native search for live files.
